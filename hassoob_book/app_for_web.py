...
st.title('Hasoup Books - Personal Assistant')
st.divider()
...
if 'generated' not in st.session_state:
    st.session_state['generated'] = ['Hello ! I am your Personal assistant built Hasoub Books Store']

if 'past' not in st.session_state:
    st.session_state['past'] = ['Hey ! 👋']

def conversational_chat(prompt):
    result = chain({'quistion':prompt,'chat_history':st.session_state['history']})
    st.session_state['history'].append((prompt,result['answer']))
    return result['answer']

with request_container:
    with st.form(key='xyz_form', clear_on_submit=True):
        user_input = st.text_input('prompt:',placeholder='Message HasoubBot...',key='input')
...
if st.session_state['generated']:
    with response_container:
        for i in range(len(st.session_state['generated'])):
            ...
            message(st.session_state['generated'][i], key=str(i), avatar_style='bots', seed=2)
'''
import os
import openai
import streamlit as st
...
st.title("Hsoub Books - Personal Assitant")
st.divider()
...
if 'generated' not in st.session_state:
    st.session_state['generated'] = ["Hello ! I am your Personal assistant built by Hsoub Books Store"]
